    ...
    <>
      {!chatView ? (
        <div className='flex h-screen items-center justify-center gap-3'>
          ...
          />
          <button 
            className='bg-purple-700 text-white rounded p-3' 
            onClick={() => {
              setChatView(true);
        ...
        <div className='grid grid-flow-col h-screen'>
          <div className='bg-purple-700 p-4 flex flex-col gap-4'>
            {users && users.map((user) => (
              <div className='flex gap-2' key={user.userID}>
                <button 
                  className={`flex flex-col gap-1 p-3 w-full ${selectedUser && selectedUser.userID === user.userID && 'bg-purple-800'}`} 
          ...
          <div className='flex flex-col col-span-4'>
            ...
            <div className='p-6'>
              {selectedUser && users && users.map((user) => {
                if (user.userID === selectedUser.userID) {
                  return (
                    <div key={user.userID}>
                      {user.messages && user.messages.map((message, index) => (
                        <div className={`flex flex-col gap-1 mb-6 ${message.fromSelf ? 'text-green-500' : 'text-red-500'}`} key={index}>
